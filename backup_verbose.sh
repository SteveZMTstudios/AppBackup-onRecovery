#!/sbin/bash
# verbose version will print every step
############################################
...
grep '<package name=' $packages_file | while read -r line; do
    ...
    if [[ $code_path == /data/app* ]]; then
        ...
        find $code_path -type f ! -name '*.oat' ! -name '*.art' ! -name '*.odex' ! -name '*.vdex' ! -path '*lib*' -exec cp -v --parents {} $temp_backup_dir/ \;
        ...
        #        find "/data/data/$package_name/" -type f -exec cp -v --parents {} $temp_backup_dir/data_data/files \;
        
        #        mkdir -p $temp_backup_dir/data_data/databases
        find "/data/data/$package_name" -type f ! -path '*cache*' ! -path '*temp*' ! -path '*tmp*' -exec cp -v --parents "{}" $temp_backup_dir/data_data/ \;

        ui_print "- Collecting /data/user/$user_id/$package_name."
        mkdir -p $temp_backup_dir/data_user
        find "/data/user/$user_id/$package_name" -type f ! -path '*cache*' ! -path '*temp*' ! -path '*tmp*' -exec cp -v --parents "{}" $temp_backup_dir/data_user/ \;
        ui_print "- Not found is normal."

        ui_print "- Collecting /data/user_de/$user_id/$package_name."
        mkdir -p $temp_backup_dir/data_user_de
        find "/data/user_de/$user_id/$package_name" -type f ! -path '*cache*' ! -path '*temp*' ! -path '*tmp*' -exec cp -v --parents "{}" $temp_backup_dir/data_user_de/ \;
        ...
        find "/data/media/obb/$package_name" -type f ! -path '*cache*' ! -path '*temp*' ! -path '*tmp*' -exec cp -v --parents "{}" $temp_backup_dir/media_obb/ \;
        ui_print "- Not found is normal."

        ui_print "- Collecting /data/media/Android/data/$package_name."
        mkdir -p $temp_backup_dir/media_data
        find "/data/media/$user_id/Android/data/$package_name" -type f ! -path '*cache*' ! -path '*temp*' ! -path '*tmp*' -exec cp -v --parents "{}" $temp_backup_dir/media_data/ \;
        ...
        # zip backup data
        if ! tar -czvf /sdcard/EmergencyBak/${package_name}_backup.tar.gz -C $temp_backup_dir .; then
